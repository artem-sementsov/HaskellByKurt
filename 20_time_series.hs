...
additionMaybePair :: (Ord k, Real v) => Map.Map k v -> (k, Maybe v) -> Map.Map k v
additionMaybePair myMap (_, Nothing) = myMap
additionMaybePair myMap (key, Just value) =
  if isNothing oldVal
    then Map.insert key value myMap
    else Map.insert key (fromJust oldVal + value) myMap
  where
    oldVal = Map.lookup key myMap
...
additionTS :: (Real a) => TS a -> TS a -> TS a
additionTS (TS [] []) ts2 = ts2
additionTS ts1 (TS [] []) = ts1
additionTS (TS t1 v1) (TS t2 v2) = TS completeTimes combinedValues
  where
    bothTimes = mconcat [t1, t2]
    completeTimes = [minimum bothTimes .. maximum bothTimes]
    tvMap = foldl additionMaybePair Map.empty (zip t1 v1)
    additionedMap = foldl additionMaybePair tvMap (zip t2 v2)
    combinedValues = map (`Map.lookup` additionedMap) completeTimes
...
main = do
  ...
  print $ movingAverageTS tsAll 5

  print $ additionTS ts1 ts2
